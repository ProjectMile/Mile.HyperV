...
typedef struct _VMRCB
{
    ...
    // than this value.
    HV_UINT32 volatile PendingSendSize;
...
HV_STATIC_ASSERT(sizeof(VMBFS_MESSAGE_HEADER) == 8);

// Factor out the header, etc.
#define STATIC_ASSERT_VMBFS_MESSAGE_SIZE(type, size) \
    HV_STATIC_ASSERT(sizeof(type) == 8 + size);
...
// Some compilers don't like zero-length arrays.
STATIC_ASSERT_VMBFS_MESSAGE_SIZE(VMBFS_MESSAGE_GET_FILE_INFO, 0 + 2)
...
// Some compilers don't like zero-length arrays.
STATIC_ASSERT_VMBFS_MESSAGE_SIZE(VMBFS_MESSAGE_READ_FILE, 12 + 2)
...
// Some compilers don't like zero-length arrays.
STATIC_ASSERT_VMBFS_MESSAGE_SIZE(VMBFS_MESSAGE_READ_FILE_RESPONSE, 4 + 1)
...
// Some compilers don't like zero-length arrays.
STATIC_ASSERT_VMBFS_MESSAGE_SIZE(VMBFS_MESSAGE_READ_FILE_RDMA, 24 + 2)
